...
//
// See {@link https://leetcode.com/problems/median-of-two-sorted-arrays/}
...
function findMedianSortedArrays(xs: number[], ys: number[]) {
  ...
  while (left <= right) {
    ...
    // removes the decimal.
    const p1 = Math.floor((left + right) / 2); // Note that p1 needs to be calculated using left/right.
    const p2 = Math.floor((m + n + 1) / 2) - p1; // Note that p2 needs to be calculated based on total size.
    ...
    const lmax1 = p1 === 0 ? -Infinity : xs[p1 - 1]; // If xs partition is empty, lmax1 value is unused.
    const rmin1 = p1 === m ? Infinity : xs[p1]; // If xs partition is full, rmin1 value is unused.
    const lmax2 = p2 === 0 ? -Infinity : ys[p2 - 1]; // If ys partition is empty, lmax2 value is unused.
    const rmin2 = p2 === n ? Infinity : ys[p2]; // If ys partition is full, rmin2 value is unused.
