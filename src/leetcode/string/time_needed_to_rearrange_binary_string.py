...
class Solution:
    def secondsToRemoveOccurrences(self, s: str) -> int:
        ...
        Rather than simulate the replacement process, we can think about how the replacement affects a string.  In a
        simple case, "01" -> "10".  However, a newly created "10" can create more replacements.  Here's an example:
        ...
        the left of the 1.  Here there are three 0's before the 1, so it will take 3 seconds to complete the
        replacement.
        ...
        of the first "001".  In essence, the first "001" -> "100" transferred the 0's to the right, which the second
        "001" has to deal with.

        This means the number of seconds it takes to replace the rightmost "01" is equal to the number of 0's that
        appeared to the left of it, in most cases.  However, this won't work for a case like this:
        ...
        for c in s:
            ...
            # If we see a 1 and we haven't seen any 0's before this, we don't need to update the number of seconds.
            # That is, "1111..." will not result in increasing the number of seconds.
            if count == 0:
                continue

            # Here we have seen a 1, and there are some number of zeroes before it.  Keep in mind that in most cases, we
            # can calculate that count of zeroes = count of seconds.  However, each 1 requires at LEAST one second to
            # flip.
            #
